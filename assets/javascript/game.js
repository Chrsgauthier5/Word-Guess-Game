...
document.onkeyup = function (event) { //run this function when the user chooses a letter, starts the game 
    ...
    correctWord.textContent = ""; //gets rid of the answer that will be displayed when the user gets a game over


    var userGuess = event.key; // Determines which key was pressed.

    if (!/^[^a-z]*([a-z]{1})[^a-z]*$/gim.test(userGuess)) { //reg expression to check without case sensitivity if the user input is anything BUT a letter.  If it is, return invalid input and stop there.4
        alert("Invalid Input! Please pick a letter!");
        return;
    }
    // if (!/^[a-z]+$/i.test(userGuess)){ //reg expression to check without case sensitivity if the user input is anything BUT a letter.  If it is, return invalid input and stop there.4
    //     alert ("Invalid Input! Please pick a letter!"); //still allows keys like "Enter, home etc" that are only letters, but more than 1 char, so replaced this regex
    //     return;
    // }
    console.log(lettersGuessed);
    if (lettersGuessed.indexOf(userGuess) != -1) { //checks
    ...
    if (randomFood.indexOf(userGuess) == -1) { //will only decrement your guesses remaining if you don't get a correct
        guessesLeft--;
    }
    ...
    if (guessesLeft === 0) {                           // if guesses are all out, the user lost.
        correctWord.textContent = "The word you were looking for was " + randomFood;
        alert("Game Over :^(");
        newGame();
...
// if you guess a correct letter, it should not use up one of your guesses
// need to have it so user checks valid input - regex